...
import { createCurriculumClientManager, createCurriculumHttpClient } from '@monte/api';
import type { CurriculumClientManager, CurriculumTreeUnit, LessonDraftRecord } from '@monte/api';
...
const clientManager: CurriculumClientManager = createCurriculumClientManager(
...
const normalizeLessonDocument = (
  ...
  const normalized = normalizeLessonDocumentTimelines({
    ...document,
    lesson: {
      ...document.lesson,
      topicId: topic.slug,
  ...
  assertInventoryConsistency(normalized);
