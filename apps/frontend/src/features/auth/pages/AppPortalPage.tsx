...
import { getOrganizationOverview, type OrganizationOverviewResult } from '../api/organizationClient';
import { formatUserRole, isUserRole } from '../types';
                    ...
                    <For each={details().members}>
                      {(member) => (
                        <li class="flex flex-col rounded-lg border border-[rgba(12,42,101,0.1)] bg-white px-3 py-2">
                          ...
                          <p class="text-xs text-[color:var(--color-text-muted)]">
                            Role: {renderRoleLabel(member.role)} · User ID: {member.userId}
                          </p>
                        </li>
                      )}
                    </For>
