...
Staging Scope
All Monte integrations must target the staging environment until production access is explicitly approved. The environment references below call out the active staging hosts to use today and list the production hosts only for future reference. Do not issue Monte traffic to production endpoints yet.
QTI
...
Staging server base URL (active): https://qti-staging.alpha-1edtech.com/
Production server base URL (reference only): https://qti.alpha-1edtech.com
...
Staging server base URL (active): https://api.staging.alpha-1edtech.com/
Production server base URL (reference only): https://api.alpha-1edtech.com/
...
Staging server base URL (active): https://caliper-staging.alpha-1edtech.com/
Production server base URL (reference only): https://caliper.alpha-1edtech.com/
...
Staging server base URL (active): https://staging.alpha-1edtech.com/ 
Production server base URL (reference only): https://api.alpha-1edtech.com/  
...
Implementation staging base URL (active): https://timeback-staging.alpha-1edtech.com/
Implementation base URL (reference only): https://timeback.alpha-1edtech.com/ 
...
You will need a Client ID and Client Secret pair. These credentials are environment-specific (staging or production). Request these credentials via timeback@ae.studio email.
...
Supported Authentication Methods:
  - Google OAuth
  - Email/Password (new method) - Users can now sign in directly using email credentials
...
Step 2: Request Client Credentials
You will need a Client ID specific for SSO. This is different from the Client ID and Secret pairs used for Machine to Machine (API to API) authentication described in the Authentication Guide. These credentials are environment-specific (staging or production). For SSO, only a Client ID is required and provided, a Client Secret is not needed. Request these credentials via email to timeback@ae.studio
...
Option A: OIDC Flow (for Google OAuth)
...
Option B: Direct Cognito Integration (for email/password + Google OAuth)
...
https://api.alpha-1edtech.com/scalar#tag/rostering---course-components/POST/ims/oneroster/rostering/v1p2/courses/components 
Let's create topics for Unit 1, referencing Unit 1’s sourcedId in parent.
...
You should never create a ComponentResource as a sibling of a CourseComponent, as those will not be shown in the recommendations page.
...
PowerPath will automatically store and manage XP, Mastery and Accuracy metrics when using the updateStudentQuestionResponse (for powerpath-100 lessons), finalStudentAssessmentResponse (for internal quiz, unit-test, placement, or test-out lessons), and importExternalTestAssignmentResults (for external placement or test-out lessons).
...
For the 2nd attempt, same as above but 0.5x:
100% accuracy = 1.25 * 0.5 = 0.625x multiplier
80-99% accuracy = 1 * 0.5 = 0.5x multiplier
0-79% accuracy = 0x multiplier (no xp)
For the 3rd attempt, same as above but 0.25x:
100% accuracy = 1.25 * 0.25 = 0.3125x multiplier
80-99% accuracy = 1 * 0.25 = 0.25x multiplier
0-79% accuracy = 0x multiplier (no xp)
For 4th attempt onwards, always 0xp
...
Finally, PowerPath implements a surplus calculation for XP to attribute to the student per test attempt, to avoid over giving XP for the same activity (even if it contains different questions each attempt). It currently assesses before granting the calculated XP above how much XP was granted in previous attempts, combined, and only gives it the difference.

For example:
Student completes first attempt, and is calculated and granted 6 XP
The second attempt is taken, and it calculates 10 XP. It now grants only 4 XP, as it has granted 6 XP before combined, so 10 - 6 = 4.
A third attempt is completed, and 5 XP is calculated. Previous attempts granted 10 XP total, so now it grants them 5 - 10 = 0 XP (it doesn’t give negative XP)
...
Creates or updates a test-out lesson in a course. This allows integrating with external platforms (such as Edulastic or Mastery Track) for content delivery. The created lesson is placed within a CourseComponent (acting as a standalone unit of the course) which can also be customized.
...
courseIdOnFail [TO BE DEPRECATED] specifies automatic enrollment when students score below the 90% mastery threshold.
...
Utilizes a third-party endpoint, which manages tests on Edulastic or Mastery Track, to instantiate the external test for a student. This returns the credentials needed by the student to access the platform, along with a link to the test and some identifiers to enable test results retrieval later.
Applicable for external test-out, placement, or unit-test lessons.


POST /powerpath/makeExternalTestAssignment

Import External Test Assignment Results
Utilizes a third-party endpoint, which manages tests on Edulastic or Mastery Track, to retrieve test results for an ongoing/completed test on the external platform, porting the data into the OneRoster structure (or compiling available data). It utilizes the identifiers stored in the test results of the student assigned to the test to fetch the data.
Applicable for external test-out, placement, or unit-test lessons.
...
These will work over lessons with lessonType = “placement” available in the ComponentResource’s object.
...
If the student has completed all placement tests available for a subject, an exhaustedTests flag is also set to true.
...
Test Assignments
Set of endpoints to control the Test Assignment feature, based on the old Dash Test Assignments.
This feature manages a separate table containing individual test assignments given to students they can take at any moment, and can be surfaced for them on their homepages or other parts of the app.
Tests generated can be managed normally using other PowerPath endpoints, such as getAssessmentProgress, and internally use MasteryTrack for test taking. For this reason, endpoints like makeExternalTestAssignment and importExternalTestAssignmentResults work well to do its lifecycle.
Notice that currently all tests made are created as test-out lessons, and implement the same consequences of being approved in one: Marking all enrollments of the same subject and grade as completed, and enrolling in the next course of the subject if it’s available in the Subject Tracks feature.
Tests can be assigned only for users with the Student role, and don’t expire manually. There are two endpoints that manage listing test assignments too; One focused on the student POV, with many filters and rules to limit content returned, and another for admins, in case a panel for managing assignments is to be made.
There is however a major caveat to this feature because of MasteryTrack’s usage:
MasteryTrack allows us to have a single active test assignment for a student of the same subject and grade.
It is possible that a student already had a test assignment in place, and then clicked to start a placement test of the same grade and subject, in which case the makeExternalTestAssignment endpoint will fail. Same goes the other way around, doing placements first…
Reason for that is MasteryTrack will return an “existing” error, and references too what is the actual assignment it’s talking about. We won’t be using that existing assignment, and will let this fail on purpose.
Allowing it to go through can cause 2 different tests in the app to share the same Assignment ID. Once the test is finalized, only one of them will receive the accurate response, while the other stays in a limbo.
For example, a student with a Math K test in placements ready to be done gets assigned a Math K test as well. Only the first one they interact with (thus making a test assignment) will go through, while the other will fail until the first one is completed, and MasteryTrack allows us to assign a different Math K test to the student.
Create an individual test assignment
Creates a test assignment for a student, containing a lesson (ComponentResource) configured with a Resource that implements a MasteryTrack test of indicated subject and grade.
Will not allow subject+grade combinations outside the realm of available MasteryTrack tests used in Placements. I.e.:
Math :: Grades 0-11
Language and Reading :: Grades 0-12
Science and Writing :: Grades 3-8
Social Studies is currently disabled, but will be same as above when in use 
POST /powerpath/test-assignments

Update an individual test assignment
Currently only allows updating the test name of an assignment  
PUT /powerpath/test-assignments/{id}

Get a test assignment
Finds a test assignment by ID 
GET /powerpath/test-assignments/{id}

Delete a test assignment
Soft-deletes the test assignment, by setting the status to tobedeleted. Will also trigger a test invalidation on MasteryTrack in case there was already a test assignment done for the student in that lesson 
DELETE /powerpath/test-assignments/{id}

List test assignments for students
Returns a paginated list of test assignments for a student. Supports filters for:
Student ID
Assignment status
Test subject
Test grade
Useful for displaying user-faced data or test listing. Will hide any completed test from the previous day or older. And prioritizes test in progress, then not started, then completed tests last.
GET /powerpath/test-assignments

List test assignments for Admins
Same as the listing for students above, but allows not passing a student ID, which can enable and admin panel to review multiple tests assigned to multiple students in the same listing. Filters are the same as above.
Test results are here preferably sorted by assignment date, with no distinction for statuses natively. For test results, the dedicated GET endpoint for AssessmentLineItems and AssessmentResults can be called passing the lesson ID in the test assignment.
GET /powerpath/test-assignments/admin

PowerPath 100
PowerPath 100 Structure
See Creating Content guide
The PowerPath 100 endpoints expect a ComponentResource + Resource pointing to a QTI Assessment Test or external test taking tool. For using the test as a dynamic quiz (questions served one after the other until racing max score), the ComponentResource of the lesson must include a lessonType = “powerpath-100”. Here's an example of a QTI Assessment Test in the QTI API: https://staging.alpha-1edtech.com/powerpath/syllabus/c3f22bb2-e527-4e52-8b5c-289ef2bd1a16 
...
The question serving algorithm will follow these rules:
New questions will always be prioritized. The algorithm will serve questions that have not been seen by the student, respecting the chosen difficulty.
The system takes into account every single question answered by the student across all attempts taken at that test. A fresh question is picked out of it, if available.
If not, The system narrows the search to find a fresh question only within the current attempt scope. And if even that is not found, then it’ll use the whole question bank of the test as a source.
The system then applies filters by difficulty with some “back-off”:
...
For example: If the score is 30 and the student has exhausted “easy” questions, we randomly serve a “medium” question, or a “hard” question if “medium” is also exhausted.
...
Finally, the system groups the available pool of questions by human-approved or not, which’s a new optional boolean attribute in the QTI question’s metadata (metadata.humanApproved).
If that’s available for some questions, those would be randomized first.
The fallback is the other set of non-human-approved questions.
Keep in mind that in the event of the filters ending up restricting the question bank too much, a question of a different difficulty bucket may be served to the student at a score range intended for some difficulties.
For example, if the student is at score 92, intended for hard questions only, but have already exhausted the hard questions bank considering previous and current attempts, a medium or easy question may be served instead.
The system does what it can to avoid these scenarios by applying some incremental back-offs to the logic, but there could still be a limitation in the question bank itself.
...
Powerpath Quiz
Quizzes are structured very similarly to PowerPath 100 lessons, with the difference that the ComponentResource’s lessonType is ”quiz”. See the PowerPath 100 lesson specification above for more details.
...
Powerpath Test Out
Test Out lessons are structured very similarly to PowerPath 100 and Quiz lessons, with the difference that the ComponentResource’s lessonType is ”test-out”. See the PowerPath 100 and PowerPath Quiz lesson specifications above for more details.
...
The ComponentResource’s lessonType is ”placement” 
...
Resource contains information about the toolProvider (where the tests is to be taken),
If populated, then it indicates it’s an external tests, so we also have a vendorId (indicating the ID of the test in the external platform), and has a type of “interactive” so UIs built on top of it can know how to handle these resources
If null, indicates it’s a QTI test, so type will be qti, and the url will point to a QTI test, where it can be served internally to the timeback UI, for example
...
Clients should consume goals from the enrollment metadata, as it contains the final resolved values for each student.

Courses hold default metadata goals that serve as a template for all student enrollments. When no custom goals are specified for an Enrollment, the Course's goals are inherited.

Enrollments can override course goals with student-specific values, enabling personalized learning targets while maintaining course-wide defaults.

Legacy Data
The system applies default goals for legacy enrollments. If you send requests to the get enrollment route, pointing to an old enrollment id, you'll notice that the goals in the metadata are sent with the default values.

Metrics Overview

In addition to the goals key, course and enrollment metadata may contain a metrics key.

When creating or updating a course, the owner is responsible for setting totalXp and totaLessons values for a given course. 

The value will be copied to the enrollments for convenience. For now, only total xp and total number of lessons are accepted. You can find examples below.
Goals Format


The system uses these default goal values when no custom goals are provided:
...

{
 "goals": {
   ...
   "dailyAccuracy": 85,
   "dailyMasteredUnits": 3
 },
 "otherCourseMetadataValues": "keepMeHere" 
}
...

{
 ...
 "otherEnrollmentMetadataValues": "keepMeHere"
}
Metrics Format

The Enrollment values will be copied from the Course.


Course Metrics Example
...
// In this example the default values will be used to fill the remaining goals, since only custom values for dailyXp and dailyLessons were provided.
...
// PUT /ims/oneroster/rostering/v1p2/courses/{courseId}
...
// be careful to not override metadata values unrelated to goals 
...
// option 2
{
 "enrollment": {
   ...
   "metadata": {
     "goals": {
       "dailyXp": 100  // Student-specific override
     }
   }
 }
}

// for each value not sent in `metadata.goals`, we inherit the value from the Course

// in option 1, no custom value was sent for the Enrollment. Because of this, all course goals will be used.

// in option 2, the custom dailyXp in the enrollment takes precedence, and only the other values will be fetched from the Course goals
...
// PUT /ims/oneroster/rostering/v1p2/enrollments/{enrollmentId}
...
// GET /ims/oneroster/rostering/v1p2/enrollments/{enrollmentId}

{
 "enrollment": {
   "sourcedId": "enrollment-xyz789",
   "status": "active",
   "dateLastModified": "2025-01-15T10:30:00Z",
   "role": "student",
   "user": { "sourcedId": "user-123" },
...
{
course: {
title: "Course with Metrics",
	courseCode: "METRICS001",
status: "active",
org: { sourcedId: "orgId123"},
metadata: {
		otherMetadataValues: "keepMeHere"
		metrics: {
			totalXp: 2500,
			totalLessons: 120
		}
...
// Note that metrics values are included in the metadata under the "metrics" key
// Be careful with other metadata values (especially during updates). It is recommended to include them explicitly to avoid accidental overrides.

Alpha Learn Compatibility
Notice that there is always an AlphaLearn.DailyXPGoal entry being created with the value of goals.dailyXp. This is happening because the AlphaLearn metadata existed before we added support to these custom goals.
Ideally we can all use goals.dailyXp, and if it works well, we can consider deprecating AlphaLearn.DailyXPGoal
...
Task: Create the Components for the Topics for Unit 1. Store the id for the component that is Unit 1 in parent
